...
function Window:_selectTab(tab)
    ...
    if prev then
        ...
        if prev._hAccent then tween(prev._hAccent,{BackgroundTransparency=1}) end
    ...
    if tab._hAccent then tween(tab._hAccent,{BackgroundTransparency=0}) end
...
function Window:AddTab(opts)
    ...
    local hAccent=make("Frame",{
        AnchorPoint=Vector2.new(0,0.5), Position=UDim2.new(0,0,0.5,0),
        Size=UDim2.fromOffset(3,18), BackgroundColor3=C.Accent,
    ...
    local tab=setmetatable({
        _window=win,
        _hBtn=hBtn,_hLabel=hLabel,_hAccent=hAccent,_hIconElement=hIconElement,
