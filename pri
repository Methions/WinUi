...
local watermarkHolder = make("Frame",{Name="Watermark",BackgroundTransparency=1,ClipsDescendants=true,AnchorPoint=Vector2.new(0.5,0.5),Position=UDim2.new(0.5,0,0.5,24),Size=UDim2.fromOffset(156,156),ZIndex=0,Parent=sidebar})
    ...
    local fadeRoots = { controls, sidebar, content }
    ...
    make("UIListLayout",{FillDirection=Enum.FillDirection.Vertical,HorizontalAlignment=Enum.HorizontalAlignment.Right,VerticalAlignment=Enum.VerticalAlignment.Bottom,SortOrder=Enum.SortOrder.LayoutOrder,Padding=UDim.new(0,6),Parent=notificationHolder})
    ...
    if loadingEnabled and loadingLayer then
        task.defer(function()
            ...
            main.Visible=true
            if hotbar and hotbar.Parent then hotbar.Visible=true end
...
function Window:Notify(opts)
    ...
    -- Sirius-style notification slot
    local slot=make("Frame",{Name="NotificationSlot",Size=UDim2.new(1,0,0,0),AutomaticSize=Enum.AutomaticSize.Y,BackgroundTransparency=1,LayoutOrder=self._notificationOrder,ZIndex=200,Parent=holder})

    -- Main card (Sirius template)
    ...
    -- SHINES THING (traveling diagonal shimmer from Oxidelib)
    local shimmer=make("Frame",{Name="Shimmer",Size=UDim2.fromScale(1,1),ZIndex=0,BackgroundColor3=Color3.fromRGB(230,240,255),Parent=card})
    ...
    local icon=make("ImageLabel",{
        ...
        Image=opts.Image and (tonumber(opts.Image) and "rbxassetid://"..tostring(opts.Image) or tostring(opts.Image)) or style.Icon,
        ZIndex=202,
        Parent=card,
    })
    ...
    -- Interact button (click to dismiss)
    ...
    -- Calculate size from text bounds
    descLabel.Size=UDim2.new(0,241,0,descLabel.TextBounds.Y)
    card.Size=UDim2.new(1,-10,0,descLabel.TextBounds.Y+50)
    slot.Size=UDim2.new(1,0,0,card.Size.Y.Offset+6)
    ...
    local function close(reason)
        ...
        TweenService:Create(card,TweenInfo.new(0.8,Enum.EasingStyle.Quint,Enum.EasingDirection.In),{
    ...
    interact.MouseButton1Click:Connect(function() close("manual") end)

    -- Sirius-style entrance animation
    task.spawn(function()
        card.Position=UDim2.new(0.5,0,1,card.Size.Y.Offset+2)

        local waitTime=(#body*0.1)+2
        if waitTime<=1 then waitTime=2.5 elseif waitTime>10 then waitTime=10 end
        local userDur=tonumber(opts.Duration)
        if userDur~=nil then waitTime=math.max(userDur,0.5) end

        -- Notification sound (Sirius)
        local notificationSound=Instance.new("Sound")
        notificationSound.Parent=holder
        ...
        task.wait(0.1)
        TweenService:Create(card,TweenInfo.new(0.8,Enum.EasingStyle.Exponential),{BackgroundTransparency=0.35}):Play()
        TweenService:Create(stroke,TweenInfo.new(0.6,Enum.EasingStyle.Exponential),{Transparency=0.7}):Play()
        task.wait(0.05)
        TweenService:Create(icon,TweenInfo.new(0.5,Enum.EasingStyle.Exponential),{ImageTransparency=0}):Play()
        task.wait(0.04)
        TweenService:Create(titleLabel,TweenInfo.new(0.5,Enum.EasingStyle.Exponential),{TextTransparency=0}):Play()
        task.wait(0.04)
        TweenService:Create(descLabel,TweenInfo.new(0.5,Enum.EasingStyle.Exponential),{TextTransparency=0.15}):Play()
        TweenService:Create(timeLabel,TweenInfo.new(0.5,Enum.EasingStyle.Exponential),{TextTransparency=0.5}):Play()

        notificationSound:Destroy()

        if waitTime>0 then
            task.delay(waitTime,function() close("timeout") end)
        end
    end)
...
function Window:AddTab(opts)
    ...
    corner(hBtn,7); pad(hBtn,0,0,10,10)
    table.insert(win._noDrag,hBtn)

    local hRow=make("Frame",{BackgroundTransparency=1,Size=UDim2.new(1,0,1,0),ZIndex=5,Parent=hBtn})
    make("UIListLayout",{FillDirection=Enum.FillDirection.Horizontal,VerticalAlignment=Enum.VerticalAlignment.Center,SortOrder=Enum.SortOrder.LayoutOrder,Padding=UDim.new(0,8),Parent=hRow})

    local iconBadge=make("Frame",{Size=UDim2.fromOffset(18,18),BackgroundTransparency=1,LayoutOrder=1,ZIndex=6,Parent=hRow})
    local hIconElement = createIconElement(iconBadge, iconType, iconValue, 16, 7)
    ...
    local hLabel=make("TextLabel",{
        ...
        Size=UDim2.new(1,-26,1,0),LayoutOrder=2,ZIndex=6,Parent=hRow,
        TextXAlignment=Enum.TextXAlignment.Left,
    })
...
function SubTab:AddColorPicker(opts)
    ...
    local inner=make("Frame",{Position=UDim2.fromOffset(0,0),Size=UDim2.fromOffset(PW,PH),BackgroundTransparency=1,ZIndex=101,Parent=panel})
    ...
    local function setOpen(o)
        if open==o then return end
        if o then
            open=true; cg=cg+1; repo(); panel.Visible=true; tween(panel,{Size=UDim2.fromOffset(PW,PH)})
